...
Recall that the \textit{$p$-rank} of $G$ is the maximum possible rank of an elementary abelian $p$-subgroup of $G$ and equals $\dim \K(\A_p(G))+1$.
...
Note that  \cref{prank3} was not known, for the results of \cite{AschbacherSmith} do not apply for $p<5$.\todo{Quizás es una afirmación demasiado fuerte. Ejemplos concretos.}
...
\begin{definition}[{\cite[Definition 2.1]{OS}}]
 ...
 $$i_\F(H)=\frac{1}{[N_G(H):H]}(1-\chi(\mathcal{K}(\F_{>H}))).$$
\end{definition}
...
\begin{theorem}\label{prank3}
Let $G$ be a finite group of $p$-rank $3$. If $\wt{H}_*(\mathcal{A}_p(G))=0$ then $O_p(G)\neq 1$.
\begin{proof}
Suppose the statement is false and consider a counterexample $G$.
Then $X=\K(\mathcal{A}_p(G))$ is a $2$-dimensional acyclic complex.
Equipped with the conjugation action of $G$, $X$ is a $G$-complex.
Since we are assuming $O_p(G)=1$, the action is fixed point free.
...
By \cref{lemma1.2} $\mathcal{F}=\{ H\leq G/N \tq Y^H\neq \emptyset\}$ is a separating family and $Y$ is an $H$-universal $(G/N,\mathcal{F})$-complex.
...
In any case, by \cref{proposition6.4} we must have $\mathcal{F}=\mathcal{SLV}$.
By \cref{indices}, $i_\mathcal{SLV}(1)=1$.
...
With the same argument we can prove the following generalization of \cref{prank3}.

\begin{theorem}\label{genthm}
If $K$ is an acyclic and $G$-invariant $2$-dimensional subcomplex of $\K(\S_p(G))$, then $O_p(G)\neq 1$. In particular, $\K(\S_p(G))$ is contractible.
\end{theorem}
